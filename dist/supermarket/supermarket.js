...
const defaultSavedAddress = {
  name: "Zhiq",
  mobile: "8576 4206",
  street: "26 Sunshine Terrace",
  floor: "11",
  unit: "4",
  details: "",
  line: "26 Sunshine Terrace #11-4 S(535703)",
};
...
const allowedViews = new Set(["home", "categories", "listing", "detail", "favourites", "cart", "search", "checkout", "addressForm", "paymentForm"]);
const allowedCheckoutSteps = new Set(["address", "schedule", "payment"]);
const statePresets = {
  "breakfast-cart": {
    label: "Breakfast cart",
    description: "White bread, regular milk, and sweet kaya already in the cart.",
    cart: {
...
function parseCartParam(value) {
  ...
  value.split(",").forEach((entry) => {
    ...
    if (!productId || !productById.has(productId) || !Number.isFinite(qty) || qty <= 0) {
      return;
    }

    parsed[productId] = qty;
  });
...
function parseStateFromUrl() {
  ...
  const presetName = params.get("statePreset") || params.get("cartPreset");
  const preset = presetName && statePresets[presetName] ? statePresets[presetName] : null;
  const state = preset ? {
    cart: preset.cart ? { ...preset.cart } : undefined,
    savedAddress: cloneAddress(preset.savedAddress),
    savedPaymentMethod: clonePaymentMethod(preset.savedPaymentMethod),
    view: preset.view,
    checkoutStep: preset.checkoutStep,
  } : {};

  if (params.has("cart")) {
    state.cart = parseCartParam(params.get("cart"));
  }

  if (params.get("addressPreset") === "saved") {
    state.savedAddress = cloneAddress(defaultSavedAddress);
  }

  if (params.get("paymentPreset") === "saved") {
    state.savedPaymentMethod = clonePaymentMethod(defaultSavedPaymentMethod);
  }
  ...
  if (step && allowedCheckoutSteps.has(step)) {
    state.checkoutStep = step;
  }

  const slot = params.get("slot");
  if (slot) {
    state.selectedDeliverySlot = slot;
  }
...
function applyStateConfig(state = {}) {
  resetScenarioState({ keepSearch: true });
  ...
  if (state.savedAddress) {
    savedAddress = cloneAddress(state.savedAddress);
  }

  if (state.savedPaymentMethod) {
    savedPaymentMethod = clonePaymentMethod(state.savedPaymentMethod);
  }

  if (state.selectedDeliverySlot) {
    selectedDeliverySlot = state.selectedDeliverySlot;
  }

  if (state.selectedRemark) {
    selectedRemark = state.selectedRemark;
  }

  if (state.checkoutStep && allowedCheckoutSteps.has(state.checkoutStep)) {
...
installNavIcons();
const bootState = parseStateFromUrl();
applyStateConfig(bootState);
renderHome();
renderCategories();
renderListing();
renderDetail();
...
if (bootState.view && allowedViews.has(bootState.view)) {
  if (bootState.view === "listing") {
    renderListing();
    setView("listing");
  } else if (bootState.view === "detail") {
    renderDetail();
    setView("detail");
  } else {
    goToView(bootState.view);
  }
}
